  ...
  const handleAvoir = async () => {
    ...
    try {
      ...
      for (const item of cart) {
        // Ask pharmacist for the date of expiration
        const newDatePer = prompt(
          `Entrez la date de péremption (MMYYYY) pour "${item.nom_medicament}":`,
          item.datE_PER || ""
        );

        if (!newDatePer) {
          alert(`Avoir annulé pour "${item.nom_medicament}"`);
          continue;
        }

        // 1️⃣ Update stock
        const stockResp = await fetch(
          `http://localhost:7194/api/medicines/returnStock/${item.id}`,
          {
            method: "PATCH",
        ...
        if (!stockResp.ok) {
          console.error(`Erreur lors du retour de ${item.nom_medicament}`);
          continue;
        }

        // 2️⃣ Prepare sale record like handleVenteEspeces
        ventesData.push({
      ...
      if (ventesData.length > 0) {
        ...
        if (!response.ok) {
          console.error("Erreur API lors de l'enregistrement des avoirs");
          alert("⚠️ Erreur lors de l'enregistrement des avoirs");
          return;
        }

        alert("✅ Avoirs enregistrés et stock mis à jour !");
