...
export default function App() {
  ...
  const processBarcodeBuffer = () => {
    ...
    if (matches.length === 0) {
      alert(`Aucun produit trouvé pour le code ${raw}`);
      ...
      const msg =
        "⚠️ Plusieurs produits ont le même code-barres mais des dates de péremption différentes :\n\n" +
        matches
          .map(
            (m, i) =>
              `${i + 1}. ${m.nom_medicament} — Date péremption : ${
                m.datE_PER || "Non renseignée"
              }`
          )
          .join("\n") +
        "\n\nEntrez le numéro du produit que vous souhaitez ajouter.";
      ...
      if (choice) {
        ...
        if (!isNaN(idx) && idx >= 0 && idx < matches.length) {
          addToCart(matches[idx], raw);
        } else {
          alert("Numéro invalide. Aucun produit ajouté.");
        }
      }
    }
