  ...
  const processItem = async () => {
    ...
    try {
      ...
      const existing = medicines.find(
        (m) =>
          m.id === item.id &&
          (m.datE_PER === datePeremption || m.datE_PER === "")
      );
      ...
      if (existing) {
        ...
        const updated = {
          ...existing,
          Quantite: (existing.quantite || 0) + item.quantite, // Capital Q
        };
        ...
        const newMed = {
          CODE: template?.code || item.code || item.id.toString(), // Changed
          Nom_medicament: item.nom_medicament,
          FORME: template?.forme || item.forme || "", // Changed
          PRESENTATION: template?.presentation || item.presentation || "", // Changed
          PPV: item.ppv,
          PH: template?.ph || "0",
          Quantite: item.quantite, // Changed capital Q
          DATE_PER: datePeremption, // Changed
