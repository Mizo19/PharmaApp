...
type Document = {
  ...
  dateReceived: string;
  items: LineItem[];
  pushed?: boolean; // track if sent to DB
};
...
export default function GestionCommands() {
  ...
  const results =
    search.trim().length > 0
      ? medicines.filter(
          (m) =>
            (m.nom_medicament?.toLowerCase() || "").includes(
              search.toLowerCase()
  ...
  const addDocument = () => {
    ...
    const newDoc: Document = {
      nDocument,
      societe,
      dateReceived: new Date(dateReceived).toISOString(), // ✅ ISO format
      items,
    };
  ...
  const pushToDatabase = async (doc: Document, index: number) => {
    try {
      // map items to backend DTO shape
      const payload = {
        N_DOCUMENT: doc.nDocument,
        NOM_SOCIETE: doc.societe,
        DATE_RECEIVED: doc.dateReceived, // ISO string is ok if backend DateTime parses it
